...
public class DDMFormUploadFileEntryHandler implements UploadFileEntryHandler {
	...
	private void _validateFileExtension(String fileName)
		...
		for (String fileExtension : fileExtensions) {
			if (StringUtil.equals(
					StringUtil.toLowerCase(FileUtil.getExtension(fileName)),
					StringUtil.toLowerCase(StringUtil.trim(fileExtension)))) {
	...
	private void _validateFileSize(String fileName, File file)
		...
		if (fileSize > guestUploadMaximumFileSize) {
			throw new FileSizeException(
				StringBundler.concat(
					fileSize, " exceeds the maximum permitted size of ",
					guestUploadMaximumFileSize, " for file ", fileName));
		}
	}
