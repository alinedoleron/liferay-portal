...
public class EmailNotificationType extends BaseNotificationType {
	...
	private String _formatContent(
		...
		String content = _formatLocalizedContent(
			notificationTemplateRecipientSetting.getValue(), _userLocale, null,
			notificationContext);

		if (Validator.isNull(content)) {
			return _formatLocalizedContent(
				content, _siteDefaultLocale, null, notificationContext);
		}
	...
	private String _formatLocalizedContent(
		...
		String content = contentMap.get(_userLocale);

		content = _formatLocalizedContent(
			content, _userLocale, null, notificationContext);

		if (Validator.isNull(content)) {
			content = contentMap.get(_siteDefaultLocale);
